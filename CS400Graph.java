...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
    	...
    	if(!vertices.containsKey(start) && vertices.containsKey(end)) throw new NoSuchElementException();
    	
    	//shortest current path
    	Path shortest = new Path(vertices.get(start));
    	//current vertex in traversal
    	Vertex current = vertices.get(start);
    	//Queue to hold possible paths in order
    	PriorityQueue<Path> potentials = new PriorityQueue<Path>();
    	//Hashtable to hold all visited vertices
    	Hashtable<T, Vertex> visited = new Hashtable();
    	
    	while(current.data != end) {
    		for(int i=0; i<current.edgesLeaving.size(); i++) {
    			//Create new path object with current shortest path
    			Path p = new Path(shortest);
    			//add all possible edges to the current shortest path
    			p.extend(current.edgesLeaving.get(i));
    			
    			//add to the potentials queue if the vertex hasn't been visited yet
    		...
    		//throw exception if there are no possible paths
    	    if(potentials.peek() == null || potentials.peek().dataSequence.size() > vertices.size())throw new NoSuchElementException();
    		
    		shortest = potentials.poll();
    		current = shortest.end;
    	}
